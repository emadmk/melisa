...
async function getOtherServices(currentId: string): Promise<OtherService[]> {
  const services = await prisma.service.findMany({
    ...
    select: { id: true, titleFa: true, slug: true, icon: true, image: true },
    orderBy: { order: 'asc' },
    take: 4,
  })
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${service.titleFa} | Melisa Trading`,
      ...
      <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 py-4 relative z-10">
          ...
          <nav className="flex items-center gap-2 text-sm mb-8">
            ...
            <ChevronRight className="w-4 h-4 text-slate-600" />
            <span className="text-white font-medium">{service.titleFa}</span>
          </nav>
          ...
          <div className="py-8 sm:py-12">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
              ...
              <div className="text-center sm:text-left">
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-3">{service.titleFa}</h1>
                {service.shortDesc && (
